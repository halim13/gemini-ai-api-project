...
function renderChatsSidebar(chats) {
  ...
  chats.forEach((chat) => {
    ...
    // Actions Container for side-by-side buttons
    const actions = document.createElement("div")
    actions.className = "chat-item-actions"

    // Action button (SVG Pencil/Edit) to rename the chat
    const btnRename = document.createElement("button")
    btnRename.type = "button"
    btnRename.className = "btn-rename-chat"
    btnRename.title = "Ubah Nama Obrolan"
    btnRename.innerHTML = `
    ...
    btnRename.addEventListener("click", async (e) => {
      ...
      if (newTitle.trim() === "") {
        alert("Nama obrolan tidak boleh kosong.")
        return
      }
      ...
      try {
        ...
        alert("Gagal mengubah nama obrolan: " + err.message)
      }
    })
